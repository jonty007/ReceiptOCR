...
function check_market(text_body) {
    for (let i =  0; i  < MARKETS.length; i++) {
        ...
        if (text_body.toLowerCase().split(' ').includes(market)) {
            return market;
        }
...
function parseResponse(gcloudResponse) {
    ...
    for(let i = 0; i < sorted_annotations.length; i++) {
        ...
        for (let j = 0; j < STOPWORDS; j++) {
            ...
            if (annotation.description.toLowerCase().split(' ').includes(word)) {
        ...
        if (t_type === 'text') {
            ...
            for(let j = 0; j < sorted_annotations.length; j++) {
                ...
                for (let k = 0; k < STOPWORDS; k++) {
                    ...
                    if (p_ann.description.toLowerCase().split(' ').includes(word)) {
                ...
                let line_overlap = Math.min(p_ymax-ymin, ymax-p_ymin) / Math.max(p_ymax-p_ymin, ymax-ymin);
                if (line_overlap < 0.5) {
                    continue;
                } 
